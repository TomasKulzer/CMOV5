...
void log_kpm_measurements(kpm_ind_msg_format_1_t const* msg_frm_1, 
				int64_t now, int64_t collect_start)
{
...
//  }
  ...
  for (size_t j = 0; j < msg_frm_1->meas_data_lst_len; j++) {
    ...
    for (size_t z = 0; z < data_item.meas_record_len; z++) {
      ...
      if (kpm_csv != NULL && meas_type.type == NAME_MEAS_TYPE) {
        if (record_item.value == INTEGER_MEAS_VALUE) {
          fprintf(kpm_csv, "%ld,%ld,%ld,%.*s,%d\n",
                  now,
                  collect_start,
                  now - collect_start,
                  (int)meas_type.name.len,
                  (char*)meas_type.name.buf,
                  record_item.int_val);
        } else if (record_item.value == REAL_MEAS_VALUE) {
          fprintf(kpm_csv, "%ld,%ld,%ld,%.*s,%.2f\n",
                  now,
                  collect_start,
                  now - collect_start,
                  (int)meas_type.name.len,
...
int main(int argc, char* argv[])
{
  ...
  kpm_csv = fopen("kpm_results.csv", "w");
  assert(kpm_csv != NULL && "Cannot open kpm_results.csv");
  fprintf(kpm_csv, "ts_now_us,collect_start_us,latency_us,metric,value\n");
  fflush(kpm_csv);
